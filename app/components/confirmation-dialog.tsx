...
export function ConfirmationDialog({
  ...
  const handleConfirm = async () => {
    try {
      ...
      toast.error(`Erro ao enviar dados: ${error.message || "Erro desconhecido"}`);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="py-4">
          <h3 className="text-lg font-semibold mb-2">Confirme os dados antes de enviar:</h3>
              ...
              <tbody>
                {data.map((item, index) => (
                  <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    ...
                    <td className="border p-2 font-bold">{item.unidade || "un"}</td>
