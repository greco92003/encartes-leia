...
export async function POST(request: Request) {
  try {
    ...
    console.log(`Recebido ${pendingProducts.length} produtos pendentes para sincronização na aba ${SHEET_NAME}`);
    ...
    const values = pendingProducts.map((product: PendingProduct) => [
      product.productName,
      product.imageUrl,
    ]);
    ...
    try {
      ...
      console.log(`${values.length} produtos inseridos com sucesso na aba ${SHEET_NAME}: ${updatedRange}`);

      return NextResponse.json({
        success: true,
        message: `${values.length} produtos adicionados com sucesso na aba ${SHEET_NAME}`,
        addedCount: values.length,
        updatedRange,
      });
    } catch (error) {
      console.error(`Erro ao inserir produtos na aba ${SHEET_NAME}:`, error);
      return NextResponse.json(
        {
          success: false,
          message: `Erro ao adicionar produtos na aba ${SHEET_NAME}`,
          error: String(error),
        },
        { status: 500 }
      );
